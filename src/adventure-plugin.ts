...
import { ProviderImageGenerationService } from "@phantasy/agent/plugin-runtime";
import type { ImageStylePreset } from "@phantasy/agent/types";
import { kvService } from "@phantasy/agent/plugin-runtime";
import { createMediaStorageService, MediaStorageService } from "@phantasy/agent/plugin-runtime";
import { registerBuiltInProviders } from "@phantasy/agent/plugin-runtime";
  ...
  getTools(): PluginTool[] {
    ...
    return [
      ...
      },
      {
        ...
        description: "Stub: Create a short video scene (future: ComfyUI provider)",
        parameters: {
          type: "object",
          properties: {
            adventureId: { type: "string" },
        ...
        handler: async ({
          ...
          const data: AdventureData = {
            ...
            sessionId: session.id,
            userId: session.userId,
            message: `Video scene (stub) requested: ${prompt}`,
            gameState: session.gameState,
            chronicle: session.chronicle,
          };
          return {
            ...
            data: {
              sessionId: session.id,
              userId: session.userId,
              summary: "Video scene stub executed",
              adventureData: data,
            },
          };
        },
